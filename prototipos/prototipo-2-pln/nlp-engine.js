...
function renderMainMenuP2(showGreeting = false) {
  ...
  const categories = {};
  p2ActiveMenuItems.forEach((item, index) => {
  ...
  for (const [catName, items] of Object.entries(categories)) {
    ...
    categoryBlocksHtml += `
      <div style="margin-top: 0.5rem; margin-bottom: 0.35rem;">
        <span style="color: var(--accent-purple); font-size: 0.8rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em;">${catName}</span><br>
        ${itemsList}
      </div>
    `;
  }
  ...
  const menuHtml = `
    ...
    Você pode falar ou digitar livremente com suas palavras, ou escolher um <strong>número</strong>:<br>
  ...
  document.getElementById("pln-details").innerHTML = `Menu Categorizado Ativo (${p2ActiveMenuItems.length} opções disponíveis) • Digite um número [1-${p2ActiveMenuItems.length}] ou frase livre`;
}
...
async function sendMessage() {
  ...
  if (p2CurrentIntent === "CLOSED") {
    if (text === "0" || textLower === "inicio" || textLower === "início" || textLower === "voltar" || textLower === "menu" || textLower === "oi" || textLower === "olá") {
      ...
      appendBotMessage(`O atendimento anterior foi finalizado. 😊<br>Digite <strong>[0]</strong> ou clique abaixo para reiniciar o assistente.`);
      const container = document.getElementById("quick-replies");
      if (container) {
        container.innerHTML = `<button class="btn-chip" onclick="handleQuickReply('0')"><i class="bi bi-arrow-clockwise"></i> [0] Iniciar Novo Atendimento</button>`;
      }
